...
const CellCard: React.FC<{
  ...
  editable: boolean;
}> = ({ lesson, dense, onDragStart, onTogglePin, editable }) => (
  <div
    className={[
    ...
    setAllLessons((prev) => {
      ...
      if (targetIdx >= 0) {
        ...
        next[srcIdx] = {
          ...
          weekday: a.weekday,
          pair: a.pair,
          group: a.group ?? (a as any).speciality ?? "",
          time: timeOf(a.pair),
        };
      } else {
        next[srcIdx] = {
...
useEffect(() => {
  ...
  // вимірюємо ширину при mount + при resize
  const updateWidth = () => {
...
}, [groups]); // оновлюй при зміні колонок
  return (
...
>
      ...
      <div className="mt-3 flex items-center gap-3">
        ...
        <div className="text-xs text-[var(--muted)] ml-auto">
          * Стіккі «День/Пара/Час». Колонки — пагінуються, щільний режим
          стискає картки.
        </div>
      </div>
    </div>
